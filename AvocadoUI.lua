...
frame:SetScript("OnEvent", function(self, event, ...)
    ...
    -- PlayerFrameCoords = { x = -282, y = 108 }
    -- PlayerFrameCoords = { x = -300, y = 98 }
    PlayerFrameCoords = { x = -300, y = 108 }
    TargetFrameCoords = { x = PlayerFrameCoords.x + 92, y = PlayerFrameCoords.y - 42 }
    FocusFrameCoords = { x = -TargetFrameCoords.x + 32, y = TargetFrameCoords.y }
    ArenaFrameCoords = { x = 0, y = 0, padding = 50 }

    TrinketCoords = { x = 86, y = 25 }

    PlayerFrame:ClearAllPoints()
    PlayerFrame:SetPoint("CENTER", PlayerFrameCoords.x,  PlayerFrameCoords.y)
    PlayerFrame:SetScale(FrameScale)
    PlayerName:SetFont(PlayerName:GetFont(), 14, 'OUTLINE')

    TargetFrame:ClearAllPoints()
    TargetFrame:SetPoint("CENTER", TargetFrameCoords.x,  TargetFrameCoords.y)
    TargetFrame:SetScale(FrameScale)

    FocusFrame:ClearAllPoints()
    FocusFrame:SetPoint("CENTER", FocusFrameCoords.x,  FocusFrameCoords.y)
